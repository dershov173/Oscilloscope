...
далее уже сам становится наблюдаемым, посылая обьекту класса DisplayPannel отсчеты сигнала 
 */
 ...
 * @author Турнамент
 */
public class ComplexSignalDispatcher extends SignalAnalyzer{

    public ComplexSignalDispatcher(Format format){
        super(format);
...
//       try{
       //System.out.println(samplesToGenerate);
            ...
            for (int j = 0; j < samplesToGenerate; j+=2){
                result[j] = realPart[j/2];
                result[j+1] = imagePart[j/2];
            }
